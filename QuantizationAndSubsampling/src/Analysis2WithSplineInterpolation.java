...
public class Analysis2WithSplineInterpolation {
    JFrame frame;
    JLabel lbIm1;
    JLabel lbIm2;
    JLabel lbIm3;
    BufferedImage img, subsampledImgWithLinearInterpolation, subsampledImgWithAverageInterpolation, subsampledImgWithSplineInterpolation;
    ...
    public void showIms(String[] args) {
        ...
        subsampledImgWithLinearInterpolation = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        subsampledImgWithAverageInterpolation = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        subsampledImgWithSplineInterpolation = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        try {
            ...
            //upSampling
            int[][] upSampledYUV = upSampleWithAvgInterpolation(subSampledYUV, width, height);
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    subsampledImgWithAverageInterpolation.setRGB(x, y, pix);
                    ind++;
                }
            }
            ...
            //upSampling
            upSampledYUV = upSampleWithLinearInterpolation(subSampledYUV, width, height);
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    int pix = 0xff000000 | (quantizedRGB[0][ind] << 16) | (quantizedRGB[1][ind] << 8) | quantizedRGB[2][ind];
                    subsampledImgWithLinearInterpolation.setRGB(x, y, pix);
                    ind++;
                }
            }
            ...
            //upSampling
            upSampledYUV = upSamplingWithSplineInterpolation(subSampledYUV, ySubSample, uSubSample, vSubSample);
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    int pix = 0xff000000 | (quantizedRGB[0][ind] << 16) | (quantizedRGB[1][ind] << 8) | quantizedRGB[2][ind];
                    subsampledImgWithSplineInterpolation.setRGB(x, y, pix);
                    ind++;
                }
            }
        ...
        JLabel lbText1 = new JLabel("Image before minimization of artifacts with Avg Interpolation");
        lbText1.setHorizontalAlignment(SwingConstants.CENTER);
        JLabel lbText2 = new JLabel("Image after minimization of artifacts with Linear Interpolation");
        lbText2.setHorizontalAlignment(SwingConstants.CENTER);
        JLabel lbText3 = new JLabel("Image after minimization of artifacts with Cubic Interpolation");
        lbText3.setHorizontalAlignment(SwingConstants.CENTER);
        lbIm1 = new JLabel(new ImageIcon(subsampledImgWithAverageInterpolation));
        lbIm2 = new JLabel(new ImageIcon(subsampledImgWithLinearInterpolation));
        lbIm3 = new JLabel(new ImageIcon(subsampledImgWithSplineInterpolation));
    ...
    public int[][] upSamplingWithSplineInterpolation(int[][] subSampledYUV,int y, int u,int v){
        int[][] upSampledYUV = new int[3][subSampledYUV[0].length];
        upSampledYUV[0] = y!=1 ? splineInterpolation(subSampledYUV[0]) : subSampledYUV[0];
        upSampledYUV[1] = u!=1 ? splineInterpolation(subSampledYUV[1]) : subSampledYUV[1];
        upSampledYUV[2] = v!=1 ? splineInterpolation(subSampledYUV[2]) : subSampledYUV[2];
        return upSampledYUV;
    }

    public int[] splineInterpolation(int[] a){
        ...
        int xValues[] = x.stream().mapToInt(i->i).toArray();
        int yValues[] = y.stream().mapToInt(i->i).toArray();
        PolynomialSplineFunction polynomialSplineFunction = splineInterpolator.interpolate(Arrays.stream(xValues).asDoubleStream().toArray(),Arrays.stream(yValues).asDoubleStream().toArray());
        for(int i=0; i<a.length; ++i){
            result[i] = a[i] != Integer.MIN_VALUE ? a[i] : Math.round((float) polynomialSplineFunction.value(i * 1.0));
        }
        return result;
    }

    public int[][] upSampleWithAvgInterpolation(int[][] subSampledYUV, int width, int height) {
        int[][] upSampledYUV = new int[3][subSampledYUV[0].length];
        int ind = 0;
    ...
    public int[][] upSampleWithLinearInterpolation(int[][] subSampledYUV, int width, int height) {
        int[][] upSampledYUV = new int[3][subSampledYUV[0].length];
        int ind = 0;
        for (int p = 0; p < height; p++) {
            for (int q = 0; q < width; q++) {
    ...
    public static void main(String[] args) {
        Analysis2WithSplineInterpolation ren = new Analysis2WithSplineInterpolation();
        ren.showIms(args);
    }
